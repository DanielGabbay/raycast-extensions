...
export async function getOpenTabs(useOriginalFavicon: boolean): Promise<Tab[]> {
  ...
  `);
    ...
    if ((err as Error).message.includes('Can\'t get application "Google Chrome Dev"')) {
...
export async function openNewTab({
  ...
  const getOpenInProfileCommand = (profile: string) =>
    ...
    do shell script "open -na 'Google Chrome Dev' --args --profile-directory=" & profile & " " & link
  `;
...
try
    do shell script "osascript -e 'exists application \\"Google Chrome Dev\\"'"
...
export async function createNewGuestWindow(): Promise<void> {
  ...
  await runAppleScript(`
    do shell script "open -na 'Google Chrome Dev' --args --guest"
  `);
}
...
export async function createNewGuestWindowToWebsite(website: string): Promise<void> {
  ...
  await runAppleScript(`
    ...
    do shell script "open -na 'Google Chrome Dev' --args --guest " & link
  `);
}
...
export async function getActiveTab(): Promise<Tab | null> {
  ...
  try {
    const result = await runAppleScript(`
      tell application "Google Chrome Dev"
        try
          set _wnd to front window
          set _w_id to get id of _wnd as inches as string
          set _tab to active tab of _wnd
          set _title to get title of _tab
          set _url to get URL of _tab
          set _tab_index to active tab index of _wnd
          return _title & "${Tab.TAB_CONTENTS_SEPARATOR}" & _url & "${Tab.TAB_CONTENTS_SEPARATOR}" & "" & "${Tab.TAB_CONTENTS_SEPARATOR}" & _w_id & "${Tab.TAB_CONTENTS_SEPARATOR}" & _tab_index
        on error
          return ""
        end try
      end tell
